 ...
 * output class name
 */
public class VoucherBillListUI extends AbstractVoucherBillListUI {
	private static final Logger logger = CoreUIObject
			.getLogger(VoucherBillListUI.class);

	/**
	 * output class constructor
	 */
	public VoucherBillListUI() throws Exception {
		super();
	}
	 ...
	 */
	protected void menuItemPCVoucher_actionPerformed(
			java.awt.event.ActionEvent e) throws Exception {
	...
	public void auditAction_actionPerformed(ActionEvent e) throws Exception {
		...
		for (Iterator iter = pkList.iterator(); iter.hasNext();) {
			...
			VoucherBillInfo voucherBillInfo = VoucherBillFactory.getRemoteInstance().getVoucherBillInfo(new ObjectUuidPK(id));
			if (!(InterfaceBillEnum.SUBMIT).equals(voucherBillInfo.getStatus())) {
				MsgBox.showInfo("已提交状态的单据才可以审核");
	    		abort();
			}
			VoucherBillFactory.getRemoteInstance().passAudit(
...
//		super.auditAction_actionPerformed(e);
	...
	public void unAuditAction_actionPerformed(ActionEvent e) throws Exception {
		...
		for (Iterator iter = pkList.iterator(); iter.hasNext();) {
			String id = (String) iter.next();
			VoucherBillInfo voucherBillInfo = VoucherBillFactory.getRemoteInstance().getVoucherBillInfo(new ObjectUuidPK(id));
			if (!(InterfaceBillEnum.AUDIT).equals(voucherBillInfo.getStatus())) {
				MsgBox.showInfo("已审核状态的单据才可以反审核");
	    		abort();
			}
			VoucherBillFactory.getRemoteInstance().unpassAudit(
					new ObjectUuidPK(id), null);
//			String statesql = "/*dialect*/select cfbillstate from CT_CUS_Kqcx where fid = '" + voucherBillInfo.getId() + "'";
//			Object object = voucherBillInfo.get("ffivouchered");
//			if (object.equals("1")) {
//				MsgBox.showInfo("该单据已生成凭证，不能反审核！");
//	    		abort();
//			}
		}
		EcClientHelper.showOprtOK(this);
...
//		super.unAuditAction_actionPerformed(e);
	 ...
	 */
	public void actionDelPCVoucher_actionPerformed(ActionEvent e)
			throws Exception {
	...
	protected com.kingdee.eas.framework.ICoreBase getBizInterface()
			throws Exception {
		return com.kingdee.eas.custom.llwebservice.VoucherBillFactory
				.getRemoteInstance();
	}
